...
async function fetchCombinedQuakeData(startTime, endTime) {
    ...
    return uniqueSismos; // Não precisamos ordenar aqui ainda
}
...
async function calculateDailyStats() {
    ...
    // Esta lógica de escala é necessária para o gráfico. O frontend vai precisar dela.
    const depthScale = [
    ...
    // Pré-calcula os pontos para o gráfico de dispersão de 24h
    ...
    const scatterPlotPoints = sismos.map(sismo => {
        ...
        return { left, depth, size, color, info }; // Retorna apenas os dados puros
    });
...
async function calculateWeeklyStats() {
    ...
    const weeklyBarData = Object.values(dailyData).sort((a, b) => a.date - b.date).map(d => ({
        count: d.count,
        maxMag: d.maxMag,
        dayLabel: translations_en_fake.weekdays[d.date.getDay()], // O app irá traduzir isso
        dateKey: d.date.toISOString().split('T')[0]
    }));
    
    // Lógica da escala de profundidade (igual à de 24h, mas baseada nos dados de 7 dias)
    const depthScale = [
        { depth: 0, position: 0 }, { depth: 10, position: 10 }, { depth: 20, position: 20 },
        { depth: 50, position: 35 }, { depth: 100, position: 50 }, { depth: 400, position: 70 },
        { depth: 500, position: 78 }, { depth: 600, position: 86 }, { depth: 1000, position: 100 }
    ...
    // Pré-calcula os pontos para o gráfico de dispersão de 7 dias
